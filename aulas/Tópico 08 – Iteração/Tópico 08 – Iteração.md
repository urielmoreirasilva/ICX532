---
layout: default
title: "Tópico 08 – Iteração"
parent: "Aulas"
nav_order: 8
---
# Tópico 08 – Iteração [<img src="https://raw.githubusercontent.com/urielmoreirasilva/ICX532/main/aulas/T%C3%B3pico%2008%20%E2%80%93%20Itera%C3%A7%C3%A3o/images/colag_logo.svg" style="float: right; vertical-align: middle; width: 42px; height: 42px;">](https://colab.research.google.com/github/urielmoreirasilva/ICX532/blob/main/aulas/T%C3%B3pico%2008%20%E2%80%93%20Itera%C3%A7%C3%A3o/T%C3%B3pico%2008%20%E2%80%93%20Itera%C3%A7%C3%A3o.ipynb) [<img src="https://raw.githubusercontent.com/urielmoreirasilva/ICX532/main/aulas/T%C3%B3pico%2008%20%E2%80%93%20Itera%C3%A7%C3%A3o/images/github_logo.svg" style="float: right; margin-right: 12px; vertical-align: middle; width: 36px; height: 36px;">](https://github.com/urielmoreirasilva/ICX532/blob/main/aulas/T%C3%B3pico%2008%20%E2%80%93%20Itera%C3%A7%C3%A3o/T%C3%B3pico%2008%20%E2%80%93%20Itera%C3%A7%C3%A3o.ipynb)

Nesse tópico, vamos aprender um pouco sobre os _laços_ (_loops_) em Python e sobre como gerar dados.
...
- Observe que essas palavras _não estão entre aspas_!
...
Um desse uso no caso acima é avaliar `capstone == 'finished' and units >= 180`.

### Exercício ✅

Suponha que tenhamos `a = True` e `b = True`. Qual é o valor da expressão a seguir? Preencha a célula de texto abaixo com a alternativa **correta**.
...
Nota: _o espaçamento no Python é muito importante_, então o **recuo extra** no código acima deve ser sempre respeitado!
...
Utilizando as estruturas condicionais, complete na célula de código abaixo a definição da função `grade_converter` abaixo, que recebe uma nota em 100 (`grade`) e retorna o conceito correspondente (de `A` a `B`), conforme a seguinte tabela:
...
Sem executar nenhum código, responda nas duas células de texto que seguem:
...
![](https://raw.githubusercontent.com/urielmoreirasilva/ICX532/main/aulas/T%C3%B3pico%2008%20%E2%80%93%20Itera%C3%A7%C3%A3o/images/iteration.png)
...
Note que em nenhum lugar atribuímos `num` a eesses elementos com `num =`!

### Exercício ✅

Usando o array `colleges` definido abaixo, escreva na célula de código que segue um loop `for` que imprima:
...
![](https://raw.githubusercontent.com/urielmoreirasilva/ICX532/main/aulas/T%C3%B3pico%2008%20%E2%80%93%20Itera%C3%A7%C3%A3o/images/append.jpg)
...
- Normalmente utilizamos o `np.append` dentro de um loop `for` da seguinte forma: a cada iteração, invocamos `nome_do_array = np.append(nome_do_array, element_to_add)`, e o array aumenta em uma linha.
...
```python
## É necessário re-atribuir (ou "salvar") o novo array!
...
```python
## Note que o array `heads_array` inicia esse processo vazio,
## -- pois antes de iniciar a simulação, ainda não lançamos nenhuma moeda!
...
```python
## Execute a célula acima e verifique como esse array se modifica!
heads_array
```
...
![png](T%C3%B3pico%2008%20%E2%80%93%20Itera%C3%A7%C3%A3o_files/T%C3%B3pico%2008%20%E2%80%93%20Itera%C3%A7%C3%A3o_107_0.png)
    


### Acumuladores

![](https://raw.githubusercontent.com/urielmoreirasilva/ICX532/main/aulas/T%C3%B3pico%2008%20%E2%80%93%20Itera%C3%A7%C3%A3o/images/accumulate.jpg)
...
```python
## Exemplo de um acumulador padrão
...
- Não entraremos em detalhes técnicos aqui, mas a utilização de loops `for` para realizar operações matemáticas em DataFrames/arrays/séries é _muito ineficiente do ponto de vista computacional_.
    - Isso decorre do fato de que o Python é uma linguagem _interpretada_.
    - O pessoal do DSC10 preparou um [breve vídeo](https://www.youtube.com/watch?v=BlczSBT80fU) sobre o assunto 🎥.

- Na prática, o ideal é sempre manipularmos `array`s, `DataFrame`s e `Series` utilizando os métodos e funções especificamente desenvolvidos para tal: consultas, GroupBy, indexação, transmissão/vetorização, etc.
...
Complete na célula de código abaixo a implementação da função `vowel_count`, que retorna o número de vogais na string de entrada `s` (incluindo repetições).
